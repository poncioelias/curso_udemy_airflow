...
default_args = {
    ...
    'email': ['poncio@teltelecom.com.br'],
...
#schedule_interval="*/3 * * * *"  => dessa forma vai rodar a cada 3 minutos
dag = DAG('windturbine', description='Dados da Turbina', schedule_interval=None, start_date=datetime(2023,3,5),
          catchup=False, default_args=default_args, default_view='graph',
          doc_md='## Dag para registrar dados de turbina eólicas')
...
create_table = PostgresOperator(
    ...
    sql="""
        ...
        sensors ( idtemp varchar, powerfactor varchar, hydraulicpressure varchar, tempature varchar, timestamp varchar);
        """,
...
send_email_normal = EmailOperator(
    ...
    html_content='''
        <h3>Temperaturas estão normais</h3>
        <p>Dag: windturbine</p>
     ''',
     task_group=group_check_temp,
     dag=dag
...
def avalia_temp(**context):
    number = float(context['ti'].xcom_pull(task_id='get_data',key="temperature"))
    if number >= 24: 
        return 'send_email_alert'
    else:
        return 'send_email_normal'
